...
constexpr uint32 kLinkProtocol = 11; // 11: the maneuver's rest (mv <name> rest:<n>; cd p carries the percent a rest order runs to, whether she kneels, Healing's ticks, next and interval ms); 10: the ring on the mesh (cd ring <x> <y> <z> <asked x> <asked z> answers a walk the mesh moved); 8: maneuvers (mv <name> [off], mv; cd mv <name> on | cd mv <name> | cd mv; gvx carries key=mask,mp)
