-----------------------------------
-- Cardian: era vendor stock the era module does not cover
--
-- Modern vendors sell what 2007 made you earn. Each override below is a
-- vendor's stock with the post-era lines removed; the price book compiler
-- (tools/economy/gamedata.py) reads these overrides the same way it reads
-- the era module's, so the crowd and the game agree. Active only while
-- Wings of the Goddess content is disabled (the prod profile).
--
-- Amalasanda (Lower Jeuno, Tenshodo): the five quested Ichi scrolls
-- (Utsusemi, Jubaku, Hojo, Kurayami, Dokumori -- the ninja's rite of
-- passage; the auction house sells them only after the quest) and the
-- three Recall scrolls (Wings of the Goddess spells).
-----------------------------------
require('modules/module_utils')
-----------------------------------
local m = Module:new('cardian_era_vendors', xi.pre(xi.expansion.WOTG))
